...
- `humanised_text` (the NanoGPT output)
...
The backend serves an HTML admin dashboard at `GET /admin` (not under `/api/v1`). It provides:
- Thought pipeline statistics (total thoughts, resolutions, themes)
- Elasticsearch health status and index stats
- Account counts from PostgreSQL
- Anonymiser mode toggle (Ollama ↔ NanoGPT fallback)

Admin access is protected by a password printed to stderr on startup (or set via `ADMIN_PASSWORD` env var). The admin API endpoints are under `/api/v1/admin/`.
...
|-----------|------|---------|
| CORS | `middleware/cors.py` | Allows requests from `localhost:3000`, `127.0.0.1:3000`, and `capacitor://localhost` |
...
All localStorage keys that contain user-generated text (`echo_thoughts`, `echo_future_letters`, `echo_saved_anchors`) are encrypted at rest using **AES-GCM** via the Web Crypto API (`frontend/src/lib/crypto.ts`). The encryption key is derived from the user's session. Plaintext raw thoughts never sit unencrypted in localStorage on an authenticated device. A legacy plaintext migration path re-encrypts data on first read when a key becomes available.
...
### Local development
- Docker Compose (`infra/docker-compose.yml`) starts PostgreSQL, Ollama, Elasticsearch, FastAPI backend, and Next.js frontend as five services
- Health checks are defined for all services; backend depends on postgres + ollama + elasticsearch being healthy
- Elastic Cloud free tier for production; local Elasticsearch container as fallback for development
